...
public class UserDao implements Serializable{
    ...
    public User login(User user){
        User u=null;
        u=mongoTemplate.findOne(Query.query(Criteria.where("").andOperator(
                Criteria.where("u_name").exists(true),Criteria.where("u_pwd").exists(true),
                Criteria.where("u_name").is(user.getU_name()).and("u_pwd").is(user.getU_pwd()))),User.class);
        return u;
    }
    ...
    public Integer userFindCount(){
       return mongoTemplate.find(Query.query(Criteria.where("u_name").exists(true).ne(null).and("u_pwd").exists(true).ne(null)),User.class).size();
    }
